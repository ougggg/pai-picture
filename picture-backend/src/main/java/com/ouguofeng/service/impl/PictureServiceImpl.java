...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture> implements PictureService {
    ...
    public Page<PictureVO> getPictureVOPage(Page<Picture> picturePage, HttpServletRequest request) {
        ...
        if (loginUser != null) {
            ...
            List<com.ouguofeng.model.entity.PictureLike> pictureLikeList = pictureLikeMapper
                    .selectList(likeQueryWrapper);
            ...
            List<com.ouguofeng.model.entity.PictureFavorite> pictureFavoriteList = pictureFavoriteMapper
                    .selectList(favoriteQueryWrapper);
    ...
    @Override
    public CreatePortraitStyleRedrawTaskResponse createPicturePortraitStyleRedrawTask(
     ...
     */
    private Picture uploadPictureWithoutAI(Object inputSource, PictureUploadRequest pictureUploadRequest,
            User loginUser) {
        if (inputSource == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "上传图片不能为空");
        }
    ...
    private void asyncRecognizeImageTagsAndCategory(Picture picture) {
        try {
            ...
            AliYunAiApi.ImageRecognitionResult recognitionResult = aliYunAiApi
                    .recognizeImageTagsAndCategory(picture.getUrl());
    ...
    @Override
    public void savePictureToPrivate(PictureSaveToPrivateRequest pictureSaveToPrivateRequest, User loginUser) {
        ThrowUtils.throwIf(pictureSaveToPrivateRequest == null, ErrorCode.PARAMS_ERROR);
        ThrowUtils.throwIf(loginUser == null, ErrorCode.NO_AUTH_ERROR);

        Long pictureId = pictureSaveToPrivateRequest.getPictureId();
        Long spaceId = pictureSaveToPrivateRequest.getSpaceId();
        ThrowUtils.throwIf(pictureId == null || spaceId == null, ErrorCode.PARAMS_ERROR);
        ...
        // 重置统计数据
        newPicture.setLikeCount(0);
        newPicture.setFavoriteCount(0);
        ...
        // 6. 开启事务保存并更新额度
        transactionTemplate.execute(status -> {
            boolean result = this.save(newPicture);
    ...
    @Override
    public void publishPictureToPublic(PicturePublishToPublicRequest picturePublishToPublicRequest, User loginUser) {
        ThrowUtils.throwIf(picturePublishToPublicRequest == null, ErrorCode.PARAMS_ERROR);
        ThrowUtils.throwIf(loginUser == null, ErrorCode.NO_AUTH_ERROR);

        Long pictureId = picturePublishToPublicRequest.getPictureId();
        ...
        // 只能从私有空间发布到公共空间
        if (oldPicture.getSpaceId() == null) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "该图片已在公共空间");
        }
        ...
        BeanUtils.copyProperties(oldPicture, newPicture);
        newPicture.setId(null);
        newPicture.setSpaceId(null); // 公共空间
        newPicture.setUserId(loginUser.getId());
        newPicture.setCreateTime(new Date());
        newPicture.setEditTime(new Date());
        newPicture.setUpdateTime(new Date());
